...
/*     */ 
/*     */ public class TileEntityWoodConstruct
...
/*  29 */     int dd = PlankDetailLevel * PlankDetailLevel;
/*  30 */     Arrays.fill(this.solidCheck, true);
/*     */     int layer;
/*  32 */     for (layer = 0; layer < 24; layer++) {
/*     */       
/*  34 */       int type = -1;
/*  35 */       for (int index = 0; index < 8 && this.solidCheck[layer] == true; index++) {
/*     */         
/*  37 */         int in = layer * 8 + index;
/*  38 */         if (!this.data.get(in) || (type != -1 && this.woodTypes[in] != type))
/*  39 */           this.solidCheck[layer] = false; 
/*  40 */         type = this.woodTypes[in];
/*     */       } 
/*     */     } 
/*  43 */     for (layer = 0; layer < 24; layer++) {
/*     */       
/*  45 */       int type = -1;
/*  46 */       for (int index = 0; index < 8 && this.solidCheck[layer + 24] == true; index++) {
/*     */         
/*  48 */         int in = index * 8 + layer;
/*  49 */         if (!this.data.get(in) || (type != -1 && this.woodTypes[in] != type))
/*  50 */           this.solidCheck[layer + 24] = false; 
/*  51 */         type = this.woodTypes[in];
/*     */       } 
/*     */     } 
/*     */   }
/*     */ 
...
/*  79 */     ArrayList<ItemStack> list = new ArrayList<ItemStack>();
...
/* Location:              D:\workdir\java\tfc\Source_other\[1.7.10]TerraFirmaCraft_client.jar!\com\bioxx\tfc\TileEntities\TileEntityWoodConstruct.class
